...
func CekNotifikasiEmail() {
	sqlStatement := "SELECT id," +
		"ifnull(concat(left(no_rekening,3),replace(no_rekening,left(no_rekening,length(no_rekening)-3),'XXX')),'') AS no_rekening," +
		"ifnull(nama_nasabah,'') nama_nasabah,ifnull(keterangan,'') keterangan,coalesce(nominal,0) nominal," +
		"ifnull(my_kode_trans,'') my_kode_trans,ifnull(date(tgl_trans),date(now())),ifnull(modul,'') modul,ifnull(hp,'') hp,ifnull(status_wa,0) status_wa,time(jam) as jam,ifnull(email,'') email,ifnull(status_email,0) status_email,ifnull(subject,'') subject " +
		"FROM wa_notifikasi " +
...
func CekOutboxWA() {
	...
	for rows.Next() {
		...
		ret := PostMessageWA(messageList.MessageNumber, messageList.MessageText, messageList.MessageId)
		if ret > 0 {
			functions.Logger().Info("Successfully Send Message to API " + strconv.Itoa(messageList.MessageId) + "")
		} else {
			functions.Logger().Info("Failed Send Message to API " + strconv.Itoa(messageList.MessageId) + "")
		}
	}
	rows.Close()
}
...
func CekOutboxEmail() {
	...
	for rows.Next() {
		...
		ret := PostMessageEmail(messageList.MessageEmail, messageList.MessageSubject, messageList.MessageContent, messageList.MessageId)
		if ret > 0 {
	...
	if err != nil {
		if resp.StatusCode == http.StatusOK {
			...
			if (objmap["response_code"]) == "00" {
				...
				functions.Logger().Info("Failed Send Message to API")
				return 1
			}
		...
		functions.Logger().Error("An Error Occured To API")
		return 0
	}
}

func PostMessageEmail(Email string, Subject string, Content string, id int) int {
	functions.Logger().Info("Starting Send Message to API")
